...
use crate::hci::constants::{EVT_LE_META_EVENT, EVT_DISCONN_COMPLETE, EVT_CMD_COMPLETE, EVT_CMD_STATUS, EVT_LE_CONN_COMPLETE, OGF_LE, OCF_LE_SET_SCAN_PARAMETERS, OCF_LE_CREATE_CONNECTION};
use crate::att::{
    AttClient, AttError, AttErrorCode, AttPermissions, AttResult, 
    SecurityLevel, ATT_MAX_MTU, ATT_HANDLE_MIN, ATT_HANDLE_MAX, ATT_DEFAULT_MTU,
    ATT_CID, PRIMARY_SERVICE_UUID, CHARACTERISTIC_UUID, CLIENT_CHAR_CONFIG_UUID,
    AttributeData, HandleUuidPair, ReadByTypeRequest, ReadByGroupTypeRequest,
    FindInformationRequest, ReadRequest, WriteRequest, PrepareWriteRequest,
    ExecuteWriteRequest, HandleValueNotification, HandleValueIndication,
    HandleValueConfirmation, PrepareWriteResponse, ExecuteWriteResponse,
    ReadResponse, ReadBlobRequest, ReadBlobResponse, ReadMultipleRequest, ReadMultipleResponse,
    FindByTypeValueRequest, FindByTypeValueResponse
};
use crate::att::AttOpcode;
use crate::gap::BdAddr;
use crate::l2cap::{L2capManager, /*L2capError,*/ ConnectionType};
use crate::gatt::types::{Service, Characteristic, CharacteristicProperty, Uuid};
...
pub struct GattClient {
    ...
    notification_callback: Option<Arc<Mutex<dyn Fn(u16, &[u8]) -> Result<(), GattError> + Send + Sync + 'static>>>,
}
...
impl std::fmt::Debug for GattClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GattClient")
            ...
            .field("has_connection_callback", &self.connection_callback.is_some())
            .field("has_notification_callback", &self.notification_callback.is_some())
            .finish()
    }
}
...
impl GattClient {
        ...
        F: Fn(u16, &[u8]) -> Result<(), GattError> + Send + Sync + 'static,
    {
        ...
        if let Some(att_client) = &self.att_client {
            ...
            att_client.set_notification_callback(move |handle, value| {
                match notification_callback.lock().unwrap()(handle, value) {
                    Ok(()) => Ok(()),
                    Err(err) => match err {
    ...
    pub fn connect(&mut self, addr: [u8; 6], addr_type: u8) -> Result<(), GattError> {
        ...
        self.socket.send_command(&conn_params)
            .map_err(|e| GattError::HciError(e.to_string()))?;
        ...
        if status != 0 {
            self.update_state(ConnectionState::Disconnected, 0);
            return Err(GattError::HciError(format!("Create connection command failed with status: {}", status)));
        }
    ...
    pub fn disconnect(&mut self) -> Result<(), GattError> {
        if let Some(handle) = self.connection_handle {
            ...
            let status = event.parameters[0];
            if status != 0 {
                return Err(GattError::HciError(format!("Disconnect command failed with status: {}", status)));
            }
    ...
    fn handle_connection_complete(&mut self, event: LeConnectionComplete) -> Result<(), GattError> {
        if event.status == 0 {
            ...
            if let Some(addr) = self.remote_addr {
                ...
                // Connect ATT channel
                att_client.connect(event.connection_handle).map_err(GattError::AttError)?;
    ...
    pub fn discover_characteristics(
        ...
        loop {
            ...
            for (handle, value) in result {
                ...
                let properties = CharacteristicProperty(value[0]);
    ...
    pub fn read_characteristic(
        ...
        let value = att_client.read(characteristic.value_handle).map_err(GattError::AttError)?;
    ...
    pub fn write_characteristic(
        ...
        att_client.write(characteristic.value_handle, data).map_err(GattError::AttError)?;
    ...
    pub fn write_characteristic_without_response(
        ...
        att_client.write_command(characteristic.value_handle, data).map_err(GattError::AttError)?;
    ...
    pub fn find_characteristic(&self, service: &Service, uuid: &Uuid) -> Option<Characteristic> {
        ...
        characteristics.get(&service.start_handle)
    ...
    pub fn enable_notifications(
        ...
        let result = att_client.find_information(
        ...
        att_client.write(cccd_handle, &[0x01, 0x00]).map_err(GattError::AttError)?;
        
        Ok(())
    }
    
    /// Enable indications for a characteristic
    pub fn enable_indications(
        &self,
        characteristic: &Characteristic
    ) -> Result<(), GattError> {
        if self.state != ConnectionState::Connected {
        ...
        att_client.write(cccd_handle, &[0x02, 0x00]).map_err(GattError::AttError)?;
    ...
    pub fn disable_notifications_and_indications(
        ...
        att_client.write(cccd_handle, &[0x00, 0x00]).map_err(GattError::AttError)?;
    ...
    fn handle_att_pdu(&mut self, pdu: &[u8]) -> AttResult<()> {
        ...
        trace!("Received ATT PDU: Opcode=0x{:02X}, Data={:?}", opcode_byte, data);
        ...
        match AttOpcode::try_from(opcode_byte) {
            Ok(AttOpcode::ReadByTypeResponse) => {
                ...
                while current_pos + length <= data.len() {
                    let decl_handle = u16::from_le_bytes([data[current_pos], data[current_pos + 1]]);
                    ...
                    let properties = CharacteristicProperty(decl_value[0]);
                    ...
                    match uuid_result {
                        ...
                        Err(_) => { warn!("Invalid UUID found in characteristic declaration"); }
                    }
                    current_pos += length;
                }
                ...
                if let Some(discovery_state) = pending_discovery_guard.as_mut() {
                    if let DiscoveryState::DiscoveringCharacteristics(service_idx) = discovery_state {
                        ...
                        warn!("Received ReadByTypeResponse for characteristics in unexpected state");
                    }
                } else {
            ...
            Ok(AttOpcode::ReadByGroupTypeResponse) => {
                 if data.len() < 1 { return Err(AttError::InvalidPdu); }
                 let length = data[0] as usize; 
                 if (length < 6) || (data.len() < 1 + length) { return Err(AttError::InvalidPdu); }
                 let mut services = Vec::new();
                 let mut current_pos = 1;
                 while current_pos + length <= data.len() {
                    let start_handle = u16::from_le_bytes([data[current_pos], data[current_pos + 1]]);
                    let end_handle = u16::from_le_bytes([data[current_pos + 2], data[current_pos + 3]]);
                    ...
                    let uuid_result: Result<Uuid, AttError> = if uuid_bytes.len() == 2 {
                     ...
                     match uuid_result {
                        Ok(uuid) => {
                         ...
                         Err(_) => { warn!("Invalid UUID found in group response"); }
                    }
                    current_pos += length;
                 }
                 let mut discovered_services_guard = self.discovered_services.lock().unwrap();
            ...
            Ok(AttOpcode::PrepareWriteResponse) => {
                 ...
                 if let Some(req) = requests.front_mut() {
                     if req.opcode == AttOpcode::PrepareWriteRequest {
                         ...
                         warn!("Received PrepareWriteResponse for non-prepare-write request: {:?}", req.opcode);
            ...
            Ok(AttOpcode::ExecuteWriteResponse) => {
                 ...
                 if let Some(req) = requests.front_mut() {
                     if req.opcode == AttOpcode::ExecuteWriteRequest {
                         ...
                         warn!("Received ExecuteWriteResponse for non-execute-write request: {:?}", req.opcode);
                         Err(AttError::UnexpectedResponse)
                     }
                 } else {
            ...
            Ok(AttOpcode::HandleValueNotification) => {
                 if pdu.len() < 3 { // Opcode + Handle (2) + Value (at least 0)
                     return Err(AttError::InvalidPdu);
                 }
                 let handle = u16::from_le_bytes([pdu[1], pdu[2]]);
                 let value = &pdu[3..];
                 info!("Received Notification: Handle=0x{:04X}, Value={:?}", handle, value);
            ...
            Ok(AttOpcode::HandleValueIndication) => {
                 ...
                 let handle = u16::from_le_bytes([pdu[1], pdu[2]]);
                 let value = &pdu[3..];
                 info!("Received Indication: Handle=0x{:04X}, Value={:?}", handle, value);
             ...
             Ok(AttOpcode::ErrorResponse) => {
                 ...
                 error!("ATT Error Response: ReqOpcode=0x{:02X}, Handle=0x{:04X}, ErrorCode={:?}", req_opcode_byte, handle, error_code);
            ...
            Ok(unhandled_opcode) => {
                ...
                if let Some(req) = requests.front_mut() {
                    if req.opcode != AttOpcode::ErrorResponse { 
                        warn!("Received unexpected opcode {:?} while waiting for {:?}", unhandled_opcode, req.opcode);
                        let callback = req.callback.take().ok_or(AttError::InvalidState)?;
