...
pub struct GattService {
    ...
    pub is_primary: bool,
    /// Store handles instead of the full struct to avoid Clone issues
    pub characteristic_handles: Vec<u16>, // Store value handles
    /// Service end handle
    pub end_handle: u16,
}
...
impl GattServer {
    ...
    pub fn add_service(&self, uuid: Uuid, is_primary: bool) -> AttResult<u16> {
        ...
        let service = GattService {
            ...
            uuid,
            is_primary,
            characteristic_handles: Vec::new(),
    ...
    pub fn add_characteristic(
        ...
        declaration_value.extend_from_slice(&[0, 0]); // Placeholder for value handle
        if let Some(uuid16) = uuid.as_u16() {
        ...
        let mut characteristics_map = self.characteristics.write().unwrap();
        characteristics_map.insert(value_handle, characteristic);

        // Fix E0308/E0277: Store handle in service instead of cloning characteristic
        service.characteristic_handles.push(value_handle);

        service.end_handle = value_handle; // Update end handle
    ...
    pub fn add_descriptor(
        ...
        let characteristic = characteristics
            .get_mut(&characteristic_value_handle)
            .ok_or(AttError::AttributeNotFound)?;
        ...
        for service in services.values_mut() {
            // Check if this service contains the characteristic
            if service
                .characteristic_handles
                .contains(&characteristic_value_handle)
            {
                service.end_handle = handle; // Update end handle to the new descriptor's handle
                break;
            }
        }
    ...
    pub fn update_characteristic(
        ...
        self.database
            .write_by_handle(handle, value, SecurityLevel::None)?;
