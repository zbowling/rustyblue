...
mod tests {
    ...
    fn test_signaling_message_config_request() {
        ...
        // Check parsed values
        match parsed {
            SignalingMessage::ConfigureRequest { identifier, destination_cid, flags, options: parsed_options } => {
    ...
    fn test_l2cap_channel() {
        // Create a channel
        let mut channel = L2capChannel::new(0x0040, L2capChannelType::ConnectionOriented, ConnectionType::Classic);
    ...
    fn test_l2cap_manager() {
        ...
        // Register a PSM
        let data_callback = Arc::new(Mutex::new(|_data: &[u8]| -> L2capResult<()> {
            Ok(())
        }));
        ...
        // Register PSM
        let result = manager.register_psm(PSM::RFCOMM, Some(data_callback), Some(event_callback), policy);
        assert!(result.is_ok());
        
        // Test PSM registration fails for duplicate
        let data_callback2 = Arc::new(Mutex::new(|_data: &[u8]| -> L2capResult<()> {
            Ok(())
        }));
