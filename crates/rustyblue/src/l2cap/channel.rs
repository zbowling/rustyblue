...
impl L2capChannel {
    ...
    pub fn new_dynamic(local_cid: u16, psm: PSM, connection_type: ConnectionType) -> Self {
    ...
    pub fn new_le_credit_based(local_cid: u16, psm: PSM, config: LeCreditBasedConfig) -> Self {
        let mut channel = Self::new(
            local_cid,
            L2capChannelType::LeCreditBased,
            ConnectionType::LE,
        );
        channel.psm = Some(psm);
    ...
    fn handle_retransmission_data(&mut self, data: &[u8]) -> L2capResult<()> {
        if data.len() < 2 {
            return Err(L2capError::InvalidParameter(
                "Retransmission data too short".into(),
            ));
        }
