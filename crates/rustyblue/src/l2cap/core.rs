...
impl L2capManager {
    ...
    pub fn handle_packet(&self, packet: L2capPacket, hci_handle: u16) -> L2capResult<()> {
        match packet.header.channel_id {
            L2CAP_SIGNALING_CID => self.handle_signaling_packet(packet, hci_handle, false),
            L2CAP_LE_SIGNALING_CID => self.handle_signaling_packet(packet, hci_handle, true),
            _ => {
    ...
    fn handle_signaling_packet(
        ...
        match message {
            ...
            SignalingMessage::LeCreditBasedConnectionResponse {
                identifier,
                destination_cid,
                mtu,
                mps,
            ...
            SignalingMessage::LeFlowControlCredit {
                identifier,
                cid,
                credits,
            ...
            _ => {
                ...
                self.send_command_reject(
                    message.get_identifier(),
                    L2CAP_REJECT_NOT_UNDERSTOOD,
                    &[],
                    hci_handle,
                )
            }
            ...
            match transaction.transaction_type {
                ...
                _ => {
                    return Err(L2capError::ProtocolError(
                        "Unexpected connection response".into(),
                    ));
                }
            }
        } else {
    ...
    fn handle_configure_response(
        ...
        if let Some(transaction) = transaction {
            match transaction.transaction_type {
                ...
                _ => {
                    return Err(L2capError::ProtocolError(
                        "Unexpected configure response".into(),
                    ));
                }
            }
        } else {
    ...
    fn handle_disconnection_response(
        ...
        if let Some(transaction) = transaction {
            match transaction.transaction_type {
                ...
                _ => {
                    return Err(L2capError::ProtocolError(
                        "Unexpected disconnection response".into(),
                    ));
                }
            }
        } else {
    ...
    fn handle_connection_parameter_update_response(
        ...
        if let Some(transaction) = transaction {
            match transaction.transaction_type {
                ...
                _ => {
                    return Err(L2capError::ProtocolError(
                        "Unexpected parameter update response".into(),
                    ));
                }
            }
        } else {
