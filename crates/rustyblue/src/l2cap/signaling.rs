...
pub enum SignalingMessage {
    ...
    ConnectionParameterUpdateResponse { identifier: SignalId, result: u16 },
    ...
    fn serialize_config_options(options: &ConfigOptions) -> Vec<u8> {
        ...
        if let Some(rfc) = options.retransmission {
            ...
            match rfc.mode {
                ...
                RetransmissionMode::Retransmission | RetransmissionMode::EnhancedRetransmission => {
    ...
    pub fn parse(data: &[u8], is_le: bool) -> Result<Self, L2capError> {
        ...
        match cmd_header.code {
            ...
            L2CAP_CONNECTION_REQUEST => {
                ...
                let psm = PSM::from_value(psm_val).ok_or_else(|| {
                    L2capError::InvalidParameter(format!("Invalid PSM value: {}", psm_val))
                })?;

                let mut cursor = Cursor::new(&params[2..4]);
                let source_cid = cursor.read_u16::<LittleEndian>().map_err(|_| {
                    L2capError::InvalidParameter("Failed to read source CID".into())
                })?;
            ...
            L2CAP_CONNECTION_RESPONSE => {
                ...
                let mut cursor = Cursor::new(&params[0..2]);
                let destination_cid = cursor.read_u16::<LittleEndian>().map_err(|_| {
                    L2capError::InvalidParameter("Failed to read destination CID".into())
                })?;
            ...
            L2CAP_DISCONNECTION_REQUEST => {
                if params.len() < 4 {
                    return Err(L2capError::InvalidParameter(
                        "Disconnection request parameters too short".into(),
                    ));
                }
            ...
            L2CAP_DISCONNECTION_RESPONSE => {
                if params.len() < 4 {
                    return Err(L2capError::InvalidParameter(
                        "Disconnection response parameters too short".into(),
                    ));
                }
