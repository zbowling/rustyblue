...
impl SmpManager {
    ...
    pub fn handle_security_request(&self, remote_addr: BdAddr, auth_req: u8) -> SmpResult<()> {
        ...
        self.notify_event(SmpEvent::PairingRequest(
            remote_addr,
            PairingFeatures {
                io_capability: IoCapability::NoInputNoOutput,
                oob_data_present: false,
    ...
    pub fn handle_smp_packet(&self, remote_addr: BdAddr, data: &[u8]) -> SmpResult<()> {
        ...
        match command_code {
            ...
            SMP_IDENTITY_ADDRESS_INFORMATION => {
                self.handle_identity_address_information(remote_addr, data)
            }
            ...
            SMP_PAIRING_KEYPRESS_NOTIFICATION => {
                self.handle_keypress_notification(remote_addr, data)
            }
                    ...
                    match process.method {
                        ...
                        Some(PairingMethod::OutOfBand) => {
                            ...
                            if let Some(oob_data) = local_oob_data.as_ref() {
                                ...
                                return self.send_pairing_failed(
                                    remote_addr,
                                    SMP_REASON_OOB_NOT_AVAILABLE,
                                );
                            }
                        }
                        _ => {
    ...
    fn handle_pairing_confirm(&self, remote_addr: BdAddr, data: &[u8]) -> SmpResult<()> {
        ...
        if process.role == PairingRole::Initiator {
            ...
            if process.local_confirm.is_none()
                && process.tk.is_some()
                && process.local_random.is_some()
            {
                // Get preq and pres
    ...
    fn handle_identity_information(&self, remote_addr: BdAddr, data: &[u8]) -> SmpResult<()> {
        ...
        // Notify the application
        self.notify_event(SmpEvent::IdentityResolvingKeyReceived(
            remote_addr,
            identity_info.irk,
        ))?;
    ...
    fn handle_identity_address_information(
        &self,
        remote_addr: BdAddr,
        data: &[u8],
    ) -> SmpResult<()> {
    ...
    fn send_pairing_failed(&self, remote_addr: BdAddr, reason: u8) -> SmpResult<()> {
        ...
        // Notify the application
        self.notify_event(SmpEvent::PairingFailed(
            remote_addr,
            PairingFailed::new(reason).to_error(),
        ))?;
    ...
    fn send_identity_address_information(
        &self,
        remote_addr: BdAddr,
        addr_type: u8,
        bd_addr: BdAddr,
    ...
    fn send_pairing_dhkey_check(
        &self,
        remote_addr: BdAddr,
        dhkey_check: PairingDhKeyCheck,
    ) -> SmpResult<()> {
