...
pub enum HciCommand {
    ...
    LeSetRandomAddress { address: [u8; 6] },
    ...
    LeSetAdvertisingData { data: Vec<u8> },
    LeSetScanResponseData { data: Vec<u8> },
...
impl HciCommand {
    ...
    pub fn opcode_parts(&self) -> (u8, u16) {
        match self {
            ...
            Self::LeReadAdvertisingPhysicalChannelTxPower => (OGF_LE, OCF_LE_READ_ADVERTISING_PHYSICAL_CHANNEL_TX_POWER),
    ...
    fn parameters(&self) -> Vec<u8> {
        match self {
            ...
            Self::Inquiry | Self::InquiryCancel | Self::Reset | 
            Self::LeReadBufferSize | Self::LeReadLocalSupportedFeatures |
            Self::LeReadAdvertisingPhysicalChannelTxPower | Self::LeCreateConnectionCancel => vec![],
            ...
            Self::LeSetAdvertisingParameters {
                min_interval, max_interval, advertising_type,
            ...
            Self::LeCreateConnection {
                peer_addr, peer_addr_type, own_address_type,
                conn_interval_min, conn_interval_max, conn_latency,
...
impl LeAdvertisingReport {
    ...
    pub fn parse_from_event(event: &HciEvent) -> Result<Vec<Self>, crate::error::Error> {
        if event.event_code != EVT_LE_META_EVENT || event.parameters.is_empty() {
            return Err(crate::error::Error::InvalidPacket("Not an LE meta event".into()));
        }
        
        let subevent_code = event.parameters[0];
        if subevent_code != EVT_LE_ADVERTISING_REPORT {
            return Err(crate::error::Error::InvalidPacket("Not an advertising report".into()));
        }
